  ...
  try {
    ...
    if (discountCodes?.length > 0) {
      ...
      if (validCodes.length !== discountCodes.length)
        return next(
          new ValidationError(
            "Some discount codes are invalid or do not belong to you",
          ),
        );
    }
...
export const deleteProduct = async (
  ...
  try {
    ...
    return res.status(200).json({
      message:
        "Product is scheduled for deletion in 24 hours. You can restore it within this duration.",
...
export const saveDraftProduct = async (
  req: any,
  res: Response,
  next: NextFunction,
) => {
  try {
    ...
    if (!title || !slug) {
      return next(
        new ValidationError("Title and slug are required for draft!"),
      );
    }
